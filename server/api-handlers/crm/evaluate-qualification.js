// api/crm/evaluate-qualification.js
//
// Synaptech CRM - Sales-Ready Qualification Engine V1
//
// Purpose:
// - Keep authenticity separate from commercial qualification
// - Measure AI discovery completeness
// - Detect explicit buyer/counselling handoff intent
// - Apply business-unit-specific qualification gates
// - Decide whether a lead is actually sales-ready
//
// IMPORTANT:
// - CRM only
// - Does NOT modify LMS/Admin
// - Does NOT modify synaptech_leads
// - Does NOT replace score-lead.js
...
export default async function handler(
  ...
  try {
    ...
    // Use the same qualification engine as public engagement events.
    // Set CRM_USE_SHARED_QUALIFICATION_ENGINE=false for legacy fallback.
    if (
      process.env
        .CRM_USE_SHARED_QUALIFICATION_ENGINE !==
      "false"
    ) {
      const result =
    ...
    if (saveError) {
      console.error(
        "CRM qualification state save failed:",
        saveError
      );
    ...
    console.error(
      "CRM qualification evaluation error:",
      error
    );
